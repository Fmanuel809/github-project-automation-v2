...
(async () => {
  try {
    ...
    const mutationQueries = generateMutationQuery(
      resource,
      project,
      column,
      nodeId,
    ...
    if (
      (action === "delete" || action === "archive" || action === "add") &&
    ...
    if (mutationQueries.length > 1) {
      console.log(
        `✅ Card materialised into to ${column} in ${mutationQueries.length} projects called ${project}`
      );
    } else {
